...
class NationalBankParser(Bank):
    ...
    def get_currency_rate(self):
        ...
        for line in result:
            if line['cc'].strip().lower() in self.__currencies.keys():
                currency_rate['rate'].append(
                     {
                         'currency_id': self.__currencies.get(line['cc'].lower())[1],
...
if __name__ == '__main__':
    ...
    db.close()
    parser = NationalBankParser(currencies, bank_url, bank_id)
